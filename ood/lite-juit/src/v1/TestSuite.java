...
public class TestSuite implements Test{
	...
	private void addTestMethod(Method m, Vector<String> names, Constructor<?> constructor) {
		...
		if(isPublicTestMethod(m)){
			...
			Test t;
			try {
				//有了构造器就可以创建名称为name的实例了
				t = (Test) constructor.newInstance(new Object[]{name});
				list.add(t);
	...
	private boolean isTestMethod(Method m) {
		...
		return returnType.equals(Void.TYPE)&&parameterTypes.length==0&&name.startsWith("test");
	}
	...
	private void addTest(Test testSuite) {
		list.add(testSuite);
	}
	@Override
